...
string student[maxrow][studentCols] = {};
...
void OpenFile()
{
    ...
    if (myfile.is_open()) {
        int i = 0;
        while (i < maxrow && getline(myfile, line)) {
            auto commaPos = line.find(',');
            if (commaPos == string::npos) continue;
            student[i][0] = line.substr(0, commaPos);
            student[i][1] = line.substr(commaPos + 1);
            i++;
        }
        myfile.close();
    }
...
bool AddRecord()
{
    ...
    //new
    if (studentCount >= maxUsers) {
        cout << "[ERROR] Student account list is full!" << endl;
        return false;
    }
    ...
    for (int x = 0; x < maxrow; x++) {
        if (!student[x][0].empty() && student[x][0] == string(stuID)) {
    ...
    // new 
    initialPassword = validateString("Create Student Password: ");



    for (int x = 0; x < maxrow; x++) {
        if (student[x][0].empty()) {
            student[x][0] = stuID;
            student[x][1] = name;
            // new
            studentUserID[studentCount] = stuID;
            studentUserPassword[studentCount] = initialPassword;
            studentCount++;
...
bool SearchRecord(string search)
{
    ...
    for (int x = 0; x < maxrow; x++) {
        if (!student[x][0].empty() && student[x][0] == search) {
            counter++;
            cout << " " << counter << " \t " << student[x][0]
                << "          \t   " << student[x][1] << endl;
            break;
...
void SaveToFile()
{
    ...
    for (int x = 0; x < maxrow; x++) {
        if (!student[x][0].empty())
            myfile << student[x][0] << "," << student[x][1] << endl;
    }
    myfile.close();
}
...
void studentrecord()
{
    ...
    do {
        ...
        while (!(cin >> choice) || choice < 1 || choice > 9) {
            cout << "\n[ERROR] Invalid option! Enter 1-9 only: ";
            cin.clear();
            cin.ignore(1000, '\n');
        }
...
bool createBooking()
{
    ...
    do {
        ...
        if (bookingStudentID[bookingCount] == "0") {
            cout << "Booking creation cancelled." << endl;
            return false;
        }
...
bool studentIdExists(string studentId)
{
    for (int studentIndex = 0; studentIndex < maxrow; studentIndex++) {
        if (student[studentIndex][0] == studentId)
            return true;
    }
    return false;
}
...
void generateDetailedReport()
{
    ...
    for (int i = 0; i < bookingCount; i++) {
        ...
        for (int j = 0; j < maxrow; j++) {
            if (student[j][0] == bookingStudentID[i]) {
